...
class TrainerGraph():
    ...
    def training_insert(self, training_version, data_version, model_version, training_metadata):
        proj_id = self.proj_id
        if not proj_id:
            print('Project {} does not exist!'.format(self.proj_name))
            return None
        ...
        data_version = str(data_version).strip()
        model_version = str(model_version).strip()
        versioning.insert_training_version(training_id, proj_id, training_version, data_version, model_version, _training_metadata)
    ...
    def addnew_testing_version(self, training_version, prediction_dir, test_version, test_params):
        # get proj_id
        proj_id = self.proj_id
        if not proj_id:
            print('Project {} does not exist!'.format(self.proj_name))
        ...
        data_metadata = {
            'data_source': prediction_dir,
            'data_type': data_type,
            'data_version': test_version,
        }
        metadata_id = data_manager_utils.insert_data_metadata(proj_id, data_metadata)
        
        # insert predict graph results
        data_gname, ttype, fields, row_ids, _ = data_manager_utils.insert_pred_graph(proj_id=proj_id, pred_dir=prediction_dir, gname=test_version, batch_size=10000)
        
        # insert testing version
        versioning.insert_test_version_node(metadata_id, proj_id, training_version, test_version, data_gname, row_ids, test_params)
